...
def last2(string):
    res = -1#we don't count the end substring
    sb = string[-2:]
    sub_len = len(sb)
...
def occurences(text):
    map={}
    for letter in list(set(text)):
        map[letter] = text.count(letter)
    return map
...
def number2digits(number):
  return [int(n) for n in list(str(number))]
...
def pigLatin(text):
    ...
    return res[0].upper() + res[1:-1].lower()
